...
(function() {
	...
	jQuery.fn.lightbox = function(noFade, autoDisappear, noLightbox, noClose, callbackOnceOpen) {
	    ...
	    if (!this.is('#lightbox')) ac.noFade = noFade; //remember so that, if no fade, no fade on LB closure either
	    ...
	    if (centralElement.id != 'lightbox') {
		  ...
		  //force central element to be child of body (lift it out of the DOM and re-insert). This ensures centering relative to body. After, put back where it was.
			...
			if (!$(centralElement).parent().is('body')) {
	...
	/* -------------------
	| UTILITY FUNC: supporting func - on lightbox close, if CE was not a child of body, reinsert it where we grabbed it from. Remove
	| lightbox class and styling from it (and its children) in process.
	------------------- */

	function reinsertCentralElement() {
		var CE = $(jQuery.fn.lightbox.centralElement), mnfr = $('#markerNodeForReinsertion');
		CE.removeClass('lightbox_centralElement').add(CE.children()).attr('style', null);
		if (mnfr.length) mnfr.after(CE).remove();
	}
	...
	lbdialog = function(params) {
	    ...
	    else if (params.error) header = 'ERROR';
		else if (params.success) header = 'SUCCESS';
		var header = header == undefined ? '' : "<h4>"+header+"</h4>";
	    $(box).html(header+'<p>'+params.content+"</p><div style='clear: both'></div>");
